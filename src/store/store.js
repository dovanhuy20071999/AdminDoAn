...
export default new Vuex.Store({
  ...
  getters: {
    ...
    listTransferHistory: (state) => state.listTransferHistory,
  },
  mutations: {
    set(state, [variable, value]) {
      state[variable] = value;
  ...
  actions: {
    ...
    //call api transfer history
    getTransferHistoryByUser({ commit }) {
      Api.requestServer1
        .get(`${Urls.ADMIN}/${Urls.TRANSACTION_HISTORY}/${Urls.LIST}`)
        .then((response) => {
          const { data } = response;
          if (data.success) {
            commit("set", ["listTransferHistory", data.data]);
          } else {
            commit("set", ["message", data.message]);
            commit("set", ["error", true]);
          }
